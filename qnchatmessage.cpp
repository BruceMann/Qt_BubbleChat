...
void QNChatMessage::messageSetting(QNChatMessage::User_Type userType, QNChatMessage::Content_Type contentType, QString content, QString time)
{

    fontRect(content,contentType);
}
...
QSize QNChatMessage::fontRect(QString str, Content_Type contentType)
{
    m_msg = str;
    ...
    QSize size;
    if(contentType==Content_Type::Content_Text)
        size = getRealString(m_msg); // 整个的size
    else if(contentType==Content_Type::Content_Image){
        QPixmap pix(m_msg);
        size = pix.rect().size();
        size = size.width()*size.height()>320*480?QSize(480,320):size;
        size =QSize(size.width()+m_spaceWid,size.height());
        m_lineHeight = 18;
    }
...
void QNChatMessage::setImagePath(QString path)
{
    m_imagePath = path;
}
...
void QNChatMessage::paintEvent(QPaintEvent *event)
{
    ...
    if(m_userType == User_Type::User_She) {
        ...
        if(m_contentType==Content_Text){
            ...
            painter.drawPixmap(m_textRightRect,image);
        }
    }
        ...
        if(m_contentType==Content_Text){        //内容
            QPen penText;
        ...
        }else if(m_contentType==Content_Image){            //展示图片
            QPixmap image(m_imagePath);
            painter.drawPixmap(m_textRightRect,image);
        }
    }
